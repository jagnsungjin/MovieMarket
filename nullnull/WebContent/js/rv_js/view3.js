function getList(state){
		...
		$.ajax({
			...
			data : {"RCM_COMMENT_BOARD_NUM" : $("#RCM_COMMENT_BOARD_NUM").val() , 
				    state:state},
			dataType:"json",
			 ...
			 if(rdata.boardlist.length>0){
				   output += '<li class="comment_tab_item ' +  red1 + '" >'
					...
					$(rdata.boardlist).each(function(){
						var lev = this.RCM_COMMENT_RE_LEV;
						var comment_reply='';
						//레벨에 따라 왼쪽 여백줍니다.
						if(lev==1){
						...
						output += '<li id="' + this.RCM_NO + '" class="CommentItem' + comment_reply + '">'
							   + '   <div class="comment_area">'
							   + '    <img  src="' + src +'" alt="프로필 사진" width="36" height="36">'
							   + '    <div class="comment_box">'
							   + '      <div class="comment_nick_box">'
							   + '            <div class="comment_nick_info">'
							   + '               <div class="comment_nickname">' + this.RCM_ID  + '</div>'
							   + '            </div>' //comment_nick_info                  
							   + '       </div>'  // comment_nick_box
							   + '     </div>'   //comment_box
							   + '    <div class="comment_text_box">'
							   + '       <p class="comment_text_view">'
							   + '         <span class="text_comment">' + this.RCM_CONTENT + '</span>'
							   + '       </p>'
							   + '    </div>' //comment_text_box
							   + '    <div class="comment_info_box">'
							   + '      <span class="comment_info_date">' + this.RCM_REG_DATE + '</span>';
						if(lev<2){ //답글쓰기는 답글의 답글까지만 사용하도록 합니다.
						   	  output += '  <a href="javascript:replyform(' + this.RCM_NO +',' 
						   	         + lev + ',' + this.RCM_COMMENT_RE_SEQ +',' 
						   	         + this.RCM_COMMENT_RE_REF +')"  class="comment_info_button">답글쓰기</a>'
						      }
						output += '   </div>' //comment_info_box;
							   
						//글쓴이가 로그인한 경우 나타나는 더보기입니다.
                        //수정과 삭제 기능이 있습니다.							
					    if($("#loginid").val()==this.RCM_ID){  
						 output +=  '<div class="comment_tool">'
							   + '    <div title="더보기" class="comment_tool_button">'
							   + '       <div>&#46;&#46;&#46;</div>' 
							   + '    </div>'
							   + '    <div id="commentItem' +  this.RCM_NO + '"  class="LayerMore">' //스타일에서 display:none; 설정함
							   + '     <ul class="layer_list">'							   
							   + '      <li class="layer_item">'
							   + '       <a href="javascript:updateForm(' + this.RCM_NO + ')"'
							   + '          class="layer_button">수정</a>&nbsp;&nbsp;'
							   + '       <a href="javascript:del(' + this.RCM_NO + ')"'
...
$(document).ready(function() {
	...
	$('ul+.CommentWriter .btn_register').click(function() {
		var RCM_CONTENT=$('.comment_inbox_text').val();
		if(!RCM_CONTENT){ //내용없이 등록 클릭한 경우
			alert("댓글을 입력하세요");
			return;
		}
		...
		$.ajax({
			...
			data : {
				RCM_ID : $("#loginid").val(),
				RCM_CONTENT : RCM_CONTENT,
				RCM_COMMENT_BOARD_NUM : $("#RCM_COMMENT_BOARD_NUM").val(),
				RCM_COMMENT_RE_LEV : 0, //원문인 경우 comment_re_seq는 0,
									//comment_re_ref는 댓글의 원문 글번호
				RCM_COMMENT_RE_SEQ : 0
			},
			type: 'post',
			success : function(rdata) {
...
//더보기를 선택한 후 수정이나 삭제를 클릭할 경우 수정과 삭제 영역이 사라지게 합니다.
	...
	$('.CommentBox').on('click','.update', function(){
		...
		$.ajax({
			...
			data:{RCM_NO:$(this).attr('data-id'), RCM_CONTENT: content},
			success:function(rdata){
				if(rdata==1){
					getList(option);
				}//if
	...
	$(".CommentBox").on('click','.reply',function(){
		...
		$.ajax({
			...
			data : {
				...
				RCM_CONTENT : content,
				RCM_COMMENT_BOARD_NUM : $("#RCM_COMMENT_BOARD_NUM").val(),
				RCM_COMMENT_RE_LEV : $(this).attr('data-lev'),
				RCM_COMMENT_RE_REF : $(this).attr('data-ref'),
				RCM_COMMENT_RE_SEQ : $(this).attr('data-seq')
			},
			type : 'post',
			success : function(rdata) {
				if(rdata ==1) {
